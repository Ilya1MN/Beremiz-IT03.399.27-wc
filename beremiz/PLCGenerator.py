    ...
    if len(lines) > 0:
        ...
        if line_num < len(lines):
            ...
            while lines[line_num][spaces] == " ":
                spaces += 1
            indent = ""
            for i in range(spaces, nb_spaces):
                indent += " "
            for line in lines:
                if line != "":
                    compute += "%s%s\n" % (indent, line)
        ...
        if body_type in ["IL", "ST"]:
            ...
            for instance in body.getcontentInstances():
                if isinstance(instance, (OutVariableClass, InOutVariableClass, BlockClass)):
                    ...
                    #TODO Убрать повторы условий
                    if executionOrderId is not None:
                        ...
                        if connections is not None:
                            ...
                            if parameter in inout_variables:
                                ...
                                if executionOrderId is not None:
                                    ...
                                    expression = self.ComputeExpression(body, connections, False)
                    ...
                    if parameter in input_names or parameter == "EN":
                        ...
                        if connections is not None:
                            if executionOrderId is not None:
                                ...
                                expression = self.ComputeExpression(body, connections, False, parameter in inout_variables)
                            if expression is not None:
