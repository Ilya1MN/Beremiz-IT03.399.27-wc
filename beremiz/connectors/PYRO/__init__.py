...
def PYRO_connector_factory(uri, confnodesroot):
    ...
    """
    confnodesroot.logger.write(("PYRO connecting to URI : %s\n") % uri)
    ...
    if servicetype == "PYROS":
        ...
        if not os.path.exists(Pyro4.config.PYROSSL_CERTDIR):
            ...
            confnodesroot.logger.write(("PYRO using certificates in '%s' \n")
                                       % (Pyro4.config.PYROSSL_CERTDIR))
    ...
    if location.find(service_type) != -1:
        try:
            ...
            port = str(i.getPort())
            newlocation = ip + ':' + port
            confnodesroot.logger.write(("'{a1}' is located at {a2}\n").format(a1=location, a2=newlocation))
            location = newlocation
            r.close()
        except Exception as msg:
            confnodesroot.logger.write_error(("MDNS resolution failure for '%s'\n") % location)
            confnodesroot.logger.write_error(traceback.format_exc())
            return None

    # Try to get the proxy object
    try:
        uri = Pyro4.URI(schemename + ':PLCObject' + "@" + location)
        RemotePLCObjectProxy = Pyro4.Proxy(uri)

    except Exception as msg:
        confnodesroot.logger.write_error(("Connection to '%s' failed.\n") % location)
        confnodesroot.logger.write_error(traceback.format_exc())
        return None
    ...
    def PyroCatcher(func, default=None):
        ...
        def catcher_func(*args, **kwargs):
            try:
                ...
                confnodesroot.logger.write_error(("Pyro exception: %s\n") % e)
            except Exception as e:
                # confnodesroot.logger.write_error(traceback.format_exc())
                errmess = Pyro4.util.getPyroTraceback(ex_value=e)
                confnodesroot.logger.write_error("\n".join(errmess) + "\n")
                print(errmess)
                confnodesroot._SetConnector(None)
            return default
    ...
    if PyroCatcher(RemotePLCObjectProxy.GetPLCstatus)() is None:
        confnodesroot.logger.write_error(("Cannot get PLC status - connection failed.\n"))
        return None
    ...
    class PyroProxyProxy(object):
        ...
        def _PyroStartPLC(self, *args, **kwargs):
            ...
            if current_status == "Dirty":
                ...
                """
                confnodesroot.logger.write(("Force runtime reload\n"))
