...
from emulator.emulator import SoftPlcEmulationRun
...
class Beremiz(IDEFrame):
    ...
    def _init_coll_HelpMenu_Items(self, parent):
        ...
        id = wx.NewId()
        parent.Append(helpString='', id=id, kind=wx.ITEM_NORMAL, item=_('Community support'))
    ...
    def _init_ctrls(self, prnt):
        ...
        StatusToolBar.SetBackgroundColour(wx.Colour("WHITE"))
    ...
    def __init__(self, parent, projectOpen=None, buildpath=None, ctr=None, debug=True):
        ...
        if projectOpen is not None and os.path.isdir(projectOpen):
            ...
            if ctr is not None:
                ...
                self.VariableTree.Enable(True)
                self.ProjectTree.Enable(True)
                self._Refresh(PROJECTTREE, POUINSTANCEVARIABLESPANEL, LIBRARYTREE, VARIABLETREE)
        if self.EnableDebug:
    ...
    def StartLocalRuntime(self, taskbaricon=True):
        if (self.local_runtime is None) or (self.local_runtime.exitcode is not None):
            ...
            self.local_runtime.spin(acquire=False)
    ...
    def RefreshRecentProjectsMenu(self):
        try:
            recent_projects = list(map(DecodeFileSystemPath,
                                  self.GetConfigEntry("RecentProjects_Py3", [])))
        except Exception:
            recent_projects = []
        ...
        if self.CTR is not None:
            ...
            for confnode_method in self.CTR.StatusMethods:
                if "method" in confnode_method and confnode_method.get("shown", True):
                    id = wx.NewId()
                    StatusToolBar.AddTool(id, confnode_method["tooltip"], GetBitmap(confnode_method.get("bitmap", "Unknown")))
    ...
    def GetConfigEntry(self, entry_name, default):
        try:
            tmp = self.Config.Read(entry_name, ';'.join(default))
            if tmp == '':
                return ['']
            return tmp.split(';')
    ...
    def RefreshConfigRecentProjects(self, projectpath, err=False):
        try:
            recent_projects = self.GetConfigEntry("RecentProjects_Py3", [])
        except Exception:
            recent_projects = []
        ...
        self.Config.Write("RecentProjects_Py3", ';'.join(recent_projects))
        self.Config.Flush()
    ...
    def OnNewProjectMenu(self, event):
        ...
        try:
            defaultpath = DecodeFileSystemPath(self.Config.Read("lastopenedfolder_Py3"), is_base64=True)
        except Exception:
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            self.Config.Write("lastopenedfolder_Py3", os.path.dirname(projectpath))
            self.Config.Flush()
            self.ResetView()
    ...
    def OnOpenProjectMenu(self, event):
        ...
        try:
            defaultpath = self.Config.Read("lastopenedfolder_Py3")
        except Exception:
            defaultpath = os.path.expanduser("~")
            ...
            if confnode is not None:
                ...
                if menu_items is not None:
                    for text, help, callback in menu_items:
                        new_id = wx.NewId()
                        confnode_menu.Append(helpString=help, id=new_id, kind=wx.ITEM_NORMAL, item=text)
                    ...
                    for name, XSDClass, help in confnode.CTNChildrenTypes:
                        if not hasattr(XSDClass, 'CTNMaxCount') or not confnode.Children.get(name) \
                            ...
                            new_id = wx.NewId()
                            confnode_menu.Append(helpString=help, id=new_id, kind=wx.ITEM_NORMAL, text=_("Add") + " " + name)
    ...
    def AddConfNode(self, ConfNodeType, confnode=None):
        if self.CTR.CheckProjectPathPerm():
            ...
            self._Refresh(TITLE, FILEMENU, PROJECTTREE, VARIABLETREE)
