...
# Copyright (C) 2020, lamsystems-it.ru (see labels EXT::IT in the source file)
#
...
from emulator.emulator import SoftPlcEmulationRun, SoftPlcEmulationThread
...
# EXT::IT (2020)
from LibIT_EEPROM.LibIT_EEPROM_Deb import LibIT_EEPROM_DebugPrint
from LibIT_EEPROM.LibIT_EEPROM_Ee import LibIT_EEPROM_EeMap, LibIT_EEPROM_Ee_ReadMap, LibIT_EEPROM_Ee_AssignMem
...
MATIEC_ERROR_MESSAGE = re.compile(".*\.st:(\d+)-(\d+)\.\.(\d+)-(\d+): (.*)$")
ITEM_CONFNODE = 25

TARGETNAME_EMULATOR = "x86_emulator"
...
def GetFeaturesCatalog():
	return [item[0] for item in features.catalog]

def CheckYAPLCConfig():
	return "yaplcconfig" in GetFeaturesCatalog()
...
class ProjectController(ConfigTreeNode, PLCControler):
	...
	def __init__(self, frame, logger):
		...
		self.InputInterfaceInfos = None
		self.OutputInterfaceInfos = None
		self.MemoryInterfaceInfos = None
		self.emulator = None
		self.emulatorFrame = None
		self.Manager = Manager()
		#self.SettingsClient = self.Manager.dict()
		pass
	...
	def LoadLibraries(self):
		...
		for libname, clsname in features.libraries:
			if self.BeremizRoot.Libraries is None or getattr(self.BeremizRoot.Libraries,
															 "Enable_" + libname + "_Library"):
	...
	def GetDefaultTargetName(self):
		if CheckYAPLCConfig():
			targets_names = targets.GetAllNames()
			return targets_names[-1]
	...
	def NewProject(self, ProjectPath, BuildPath=None):
		...
		catalog_names = GetFeaturesCatalog()
		if CheckYAPLCConfig():
			self.CTNAddChild("yaplcconfig_0", "yaplcconfig")
		self.SaveProject()
		return None
	...
	def LoadProject(self, ProjectPath, BuildPath=None):
		...
		# EXT::IT (2020)
		LibIT_EEPROM_DebugPrint("LOAD_PROJECT")
		# EEPROM Map
		global LibIT_EEPROM_EeMap
		LibIT_EEPROM_EeMap = LibIT_EEPROM_Ee_ReadMap(self.Project, False)
		# EXT::IT (2020) End
	...
	def SaveProject(self, from_project_path=None):
		if self.CheckProjectPathPerm(False):
			...
			# EXT::IT (2020)
			LibIT_EEPROM_DebugPrint("SAVE_PROJECT")
			# EEPROM Map
			global LibIT_EEPROM_EeMap
			LibIT_EEPROM_Ee_AssignMem(self.Project, LibIT_EEPROM_EeMap, None)
			# EXT::IT (2020) End
	...
	def _getEmulationpath(self):
		return os.path.join(self._getBuildPath() + "\\Emulator", self.GetProjectName()+".dll")
	...
	def _Generate_SoftPLC(self, buildpath):
		if self._Generate_PLC_ST():
			return self._Compile_ST_to_SoftPLC(buildpath)
		return False
	...
	def _Compile_ST_to_SoftPLC(self, buildpath):
		...
		if status:
			...
			for err_line in err_result.split('\n'):
				...
				if m_result is not None:
					...
					if first_line != last_first_line or last_last_line != last_line:
						last_first_line = first_line
						last_last_line = last_line
						if warning_message_end != "":
						...
						for i, line in enumerate(f.readlines()):
							...
							if first_line <= i <= last_line:
								if last_section is not None:
									warning_message_end += "In section: " + last_section
									last_section = None  # only write section once
								warning_message_end += "%04d: %s" % (i, line)
						f.close()
				else:
					if warning_message_end != "":
						self.logger.write_warning("\n" + warning_message_end + "\n")
	...
	def GetBuilder(self):
		...
		return self.GetBuilderTarget(targetname=targetname)

	def GetBuilderTarget(self, targetname):
		"""
		Return a Builder (compile C code into machine code)
		"""
		# Get target, module and class name
		...
		if self._ProgramList is None or self._VariablesList is None:
			try:
				...
				for line in ListGroup[0]:
					...
					attrs = dict(list(zip(ProgramsListAttributeName, line.strip().split(';'))))
				...
				for line in ListGroup[1]:
					# Split and Maps each field to dictionnary entries
					attrs = dict(list(zip(VariablesListAttributeName, line.strip().split(';'))))
					# Truncate "C_path" to remove conf an resources names
	...
	def Generate_plc_debugger(self, targetname):
		"""
	...
	def Generate_plc_main(self, targetname):
		"""
		...
		locstrs = ["_".join(map(str, x)) for x in [loc for loc, Cfiles, DoCalls in
		...
		plc_main_code += targets.GetTargetCode(targetname)
		plc_main_code += targets.GetCode("plc_main_tail.c")
		return plc_main_code

	def CheckErrorInExternalVariables(self):
		Error = True
		resource_vars = []
		...
		if project is not None:
			for config in project.getconfigurations():
				for resource in config.getresource():
					resource_vars = self.GetConfigurationResourceGlobalVars("config", resource.getname())
			ErrorName = False
			ErrorType = False
			globalVars = self.GetConfigurationGlobalVars("config")
			for pou in project.getpous():
				for pouValue in self.GetEditedElementInterfaceVars("P::" + pou.getname()):
					if pouValue.Class == "External":
						for globalVar in globalVars + resource_vars:
							if pouValue.Name.lower() == globalVar.Name.lower():
								ErrorName = True
								ErrorType = (pouValue.Type == globalVar.Type)
								break
							else:
								ErrorName = False
								ErrorType = False
						if not ErrorName:
							self.logger.write_warning(_("Variable name error: \'" + pou.getname().upper() + "\' missing variable name " + pouValue.Name + "\n"))
							Error = False
						elif not ErrorType:
							self.logger.write_warning(_("Variable type error: \'" + pou.getname().upper() + "\' " + ": Type mismatch in " + pouValue.Name + "\n"))
							Error = False
		return Error

	def CheckErrorInResourse(self):
		project = self.GetProject()
		for config in project.getconfigurations():
			for resource in config.getresource():
				resources_infos = self.GetEditedResourceInfos("R::config::" + resource.getname())[0]
				for infos in resources_infos:
					if infos["Triggering"] == "Cyclic":
						return True
		self.logger.write_warning("Cycle task not defined in resources\n")
		return False

	def ResourceConvert(self, resource_name, buildpath, fast_funcs_infos, builder):
		filepath = os.path.join(buildpath, resource_name + ".c")
		resource_c_file = open(filepath, "r+")
		lines: list[str] = resource_c_file.readlines()
		resource_c_file.close()
		fast_app_code = self.GenerateNewCodeFastApp(fast_funcs_infos, builder)
		lines += fast_app_code
		resource_c_file = open(filepath, "w")
		resource_c_file.writelines(lines)
		resource_c_file.close()

		filepath = os.path.join(buildpath, "config.c")
		config_c_file = open(filepath, "r+")
		lines = config_c_file.readlines()
		config_c_file.close()
		config_c_file = open(filepath, "w")
		resourceInitFunc = resource_name.upper()+"_init__"
		for line in lines:
			config_c_file.writelines(line)
			if ("void " + resourceInitFunc + "(void);") in line:
				config_c_file.write("void AppFastInit();\n")
			elif resourceInitFunc in line:
				config_c_file.write("AppFastInit();\n")
		config_c_file.close()


	def GenerateNewCodeFastApp(self, fasttasks, builder):
		"""
		Use confnodes layout given in LocationCFilesAndCFLAGS to
		generate glue code that dispatch calls to all confnodes
		"""
		...
		FuncsRun = dict()
		TaskDict = {}
		default_interfases = self.GetPlcDefaultInterfaceName()


		for task in fasttasks:
			try:
				events = task["Event"].split("\n")

				sources = [event.split(" ")[0] for event in events]
				conditions = [event.split(" ")[1] if event.split(" ")[1] != "=" else "==" for event in events]
				modes = task["Mode"].split("\n")
				values = []
				for event in events:
					event = event.split(" ")[2].replace(",", ".")
					if event.isdigit() or isFloat(event):
						values.append(event)
					elif event == _("Falling"):
						values.append("0")
					else:
						values.append("1")
				for source, condition, value, mode in zip(sources, conditions, values, modes):
					interface_name = re.sub(r"\d+", "", source, flags = re.UNICODE)
					if interface_name in TaskDict.keys():
						TaskDict[interface_name].append(
							{"Source": source.upper(), "Condition": condition + value,
							 "Bit": task["Priority"], "Name": task["Name"].upper(), "Mode": mode})
					else:
						TaskDict[interface_name] = [
							{"Source": source.upper(), "Condition": condition + value,
							 "Bit": task["Priority"], "Name": task["Name"].upper(), "Mode": mode}]
						default_interfases.remove(interface_name)
					FuncsRun[task["Name"].upper()] = task["Priority"]
			except Exception as exc:
				self.logger.write_error(_("C Build crashed !\n"))
				self.logger.write_error(_("Event task table not fill !\n"))
				self.ResetBuildMD5()
				return False

		condition_func = builder.GenerateFastApp(TaskDict, default_interfases)

		# Generate resource, based on template
		if not self.BeremizRoot.getDisable_Extensions():
			plc_fastApp = targets.GetCode("fast_app.c") % {
				"condition_space": condition_func,
				"fast_func_declaring": "\n".join([(
													    "%(func_name)s __INSTANCE_FAST_%(priority)s;" +
													    "\n") % {"func_name": name, "priority": FuncsRun[name]}for name in FuncsRun.keys()]),
				"fast_func_init_call": "\n".join([(
														"\t%(func_name)s_init__(&__INSTANCE_FAST_%(priority)s, 0);" +
														"\n") % {"func_name": name, "priority": FuncsRun[name]} for name in FuncsRun.keys()]),
				"fast_func_run_call": "\n".join([(
														"void %(name)s" + "__run__(){\n"+
														"\t%(name)s_body__(&__INSTANCE_FAST_%(priority)s);\n"+
												 		"}") % {'name': name, "priority": FuncsRun[name]} for name in FuncsRun.keys()]),
				"func_prt_space": "\n".join([(
														"\tADD_FAST_FUNC(%(s)s" + "__run__);") % {'s': name} for name in FuncsRun.keys()])
			}
		else:
			plc_fastApp = targets.GetCode("fast_app.c") % {
				"condition_space": "\n",
	...
	def _RunEmulatorThread(self):
		self.emulator = Thread(target=SoftPlcEmulationThread, args=("localhost", self.AppFrame.runtime_port, self), daemon=True)
		self.emulator.start()

	def BuildProgressDialog(self):
		dialog = wx.ProgressDialog("Build", "Build" + "...", 100, parent=self.CTNParent, style=wx.PD_AUTO_HIDE | wx.PD_APP_MODAL | wx.PD_SMOOTH )
		dialog.Pulse()
		return dialog

	def _EmulatorKill(self):
		if self.emulator is not None:
			if self.emulator.is_alive():
				if self.emulatorFrame is not None:
					self.emulatorFrame.Close()
					self.emulator.join()
				else:
	...
	def _EmulationRun(self):
		self._Clean()
		dialog = self.BuildProgressDialog()
		self._EmulatorKill()
		buildpath = self._getBuildPath()
		builder = self.GetBuilderTarget(TARGETNAME_EMULATOR)
		if self._connector is not None:
			#self._connector.StopPLC()
			#self._connector.UnLoadPLC()
			self.AppFrame.KillLocalRuntime()
			self.KillDebugThread()
		try:
			if not os.path.exists(buildpath):
				os.mkdir(buildpath)
			self._setBuildPath(buildpath + "\\Emulator")
			self._BuildTargets(TARGETNAME_EMULATOR, buildpath + "\\Emulator")
			uri = "LOCAL://"
			self._ConnectFromUri(uri)
			self._TransferTarget(builder)
	...
	def _Build(self):
		buildpath = self._getBuildPath()
		targets_name = self.GetTarget().getcontent().getLocalTag()
		self._BuildTargets(targets_name, buildpath)

	def _BuildTargets(self, targets_name, buildpath):
		"""
		...
		# EXT::IT (2020)
		LibIT_EEPROM_DebugPrint("BUILD_PROJECT")
		# EEPROM Map
		global LibIT_EEPROM_EeMap
		LibIT_EEPROM_Ee_AssignMem(self.Project, LibIT_EEPROM_EeMap, buildpath)
		# EXT::IT (2020) End

		self.CheckErrorInExternalVariables()
		if not self.CheckErrorInResourse():
			return False
		...
		IECGenRes = self._Generate_SoftPLC(buildpath)
		self.UpdateButtons()
		...
		if not self._Generate_runtime(targetname=targets_name, buildpath=buildpath):
			return False

		# Get current or fresh builder
		builder = self.GetBuilderTarget(targets_name)

		if builder is None:
			self.logger.write_error(_("Fatal : cannot get builder.\n"))
			self.ResetBuildMD5()
			return False
		#if targets_name != TARGETNAME_EMULATOR:
		project = self.GetProject()
		config = project.getconfigurations()
		if self.InputInterfaceInfos is not None:
			if len(config) == 1:
				resource = config[0].getresource()
				if len(resource) == 1:
					resource_name = resource[0].getname()
					resource_config = self.GetEditedFastTask("R::config::" + resource_name)
					self.ResourceConvert(resource_name, buildpath, resource_config, builder)
	...
	def _Generate_runtime(self, buildpath, targetname):
	...
	def _Run_x86(self):
		self.logger.write_error("Run x86\n")

	def _Stop_x86(self):
		self.logger.write_error("Stop x86\n")

	def _Connect_x86(self):
		self.logger.write_error("Connect x86\n")

	def Transfer_x86(self):
		self.logger.write_error("Transfer x86\n")

	def _Disconnect_x86(self):
		self.logger.write_error("Disconnect x86\n")
	...
	def _OpenView(self, name=None, onlyopened=False):
		if name == "IEC code":
			...
			if self._IECRawCodeView is None:
				...
				self._IECRawCodeView = IECCodeViewer(self.AppFrame.TabsOpened, "", self.AppFrame, controler,
													 instancepath=name)
			...
			if self._ProjectFilesView is None:
				self._ProjectFilesView = FileManagementPanel(self.AppFrame.TabsOpened, self, name,
															 self._getProjectFilesPath(), True)
	...
	def _UpdateButtons(self):
		...
		if self._connector is not None and self.emulator is None:
			#self.EnableMethod("_EmulationRun", False)
			self.ShowMethod("_EmulationRun", False)
		else:
			#self.EnableMethod("_EmulationRun", True)
			self.ShowMethod("_EmulationRun", True)
	...
	def UpdateMethodsFromPLCStatus(self):
		...
		if (self.previous_plcstate != status):
			...
			if self.previous_plcstate == "Stopped" and status == "Started":
				self._connect_debug()
			#elif self.previous_plcstate == "Started" and status is "Stopped":
			#	self.
	...
	def SnapshotAndResetDebugValuesBuffers(self):
		buffers, self.DebugValuesBuffers = (self.DebugValuesBuffers,
											[list() for n in range(len(self.TracedIECPath))])
	...
	def RegisterDebugVarToConnector(self):
		...
		if self._connector is not None:
			...
			if Idxs:
				...
				self._connector.SetTraceVariablesList(list(zip(*IdxsT[0:3])))
			else:
				self.TracedIECPath = []
				...
				if function is not None:
					if buffer_list:
						try:
							...
							self.UnsubscribeDebugIECVariable(IECPath, weakcallable)
							return
					else:
	...
	def KillDebugThread(self):
		...
		if tmp_debugthread is not None:
			...
			tmp_debugthread.join(timeout=100)
			if tmp_debugthread.is_alive() and self.logger:
	...
	def _ConnectFromUri(self, uri):
		...
		try:
			con = connectors.ConnectorFactory(uri, self)
			self._SetConnector(con)
		...
		if self._connector is None:
			...
			if self.previous_plcstate in ["Started", "Stopped"]:
				a = self.DebugAvailable()
				b = self.GetIECProgramsAndVariables()
				if a and b:
	...
	def _TransferTarget(self, builder):
		...
		if self._connector.MatchMD5(str(MD5)):
			self.logger.write(
		...
		for extrafilespath in [self._getExtraFilesPath(),
			...
			extrafiles.extend([(name, extrafilespath) for name in os.listdir(extrafilespath)])
		# Send PLC on target
		if builder is not None:
	...
	StatusMethods = [
		...
		},
		{
			"bitmap": "Emulation",
			"name": _("Emulation"),
			"tooltip": _("Start target device emulation"),
			"method": "_EmulationRun",
			"shown": True,
		},
	]
	...
	def GetPlcDefaultInterfaceName(self):
		return [interfase['interface']['name'] for interfase in self.InputInterfaceInfos]