...
class ProcessLogger:
    ...
    def output(self, v):
        ...
        if not self.no_stdout:
            self.logger.write(v)
        if self.keyword is not None and self.outlimit is not None:
    ...
    def spin(self, acquire=True):
        if acquire:
            self.finishsem.acquire()
        str_outdata = []
        for data in self.outdata:
            if data != b'':
                str_outdata.append(data.decode('cp1251'))
        str_errdata = []
        for data in self.errdata:
            if data != b'':
                str_errdata.append(data.decode('cp1251'))

        return [self.exitcode, "".join(str_outdata), "".join(str_errdata)]
