...
VARIABLE_CLASSES_DICT_REVERSE = dict(
    [(value, key) for key, value in VARIABLE_CLASSES_DICT.items()])
...
class FBDVariableDialog(BlockPreviewDialog):
    ...
    def __init__(self, parent, controller, tagname, exclude_input=False):
        ...
        self.SelectedClass = ["External"]

        # Init common sizers
        self._init_sizers(4, 2, 5, None, 3, 2)
        ...
        listChoices = [_("Global Variables"), _("Local Variables")]

        self.SelectClassRadio = wx.RadioBox(self, label=_("Class:"), pos=(10, 80),
                                            choices=listChoices, majorDimension=1, style=wx.RA_SPECIFY_COLS)
        self.Bind(wx.EVT_RADIOBOX, self.OnRadioBoxSelect, self.SelectClassRadio)
        self.LeftGridSizer.Add(self.SelectClassRadio, flag=wx.GROW)
        ...
        for var_class, choice in VARIABLE_CLASSES_DICT.items():
    ...
    def RefreshNameList(self):
        ...
        for name, (var_type, value_type) in self.VariableList.items():
            if var_type != "Input" and var_type in self.SelectedClass:
    ...
    def SetValues(self, values):
        ...
        var_name = values.get("expression", None)
        if var_name is not None:
            var_type, value_type = self.VariableList.get(var_name, [None, None])
            if var_type not in self.SelectedClass:
                self.SelectClassRadio.SetSelection(1)
                self.SelectedClass = ["Input", "Output", "InOut", "Local"]
    ...
    def OnRadioBoxSelect(self, event):
        sel = self.SelectClassRadio.GetSelection()
        classFilter = None
        if sel == 0:
            classFilter = ["External"]
        elif sel == 1:
            classFilter = ["Input", "Output", "InOut", "Local"]
        if classFilter is not None:
            if self.SelectedClass != classFilter:
                self.SelectedClass = classFilter
                self.RefreshNameList()
