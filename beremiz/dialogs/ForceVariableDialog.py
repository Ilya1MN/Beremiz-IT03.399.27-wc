...
class ForceVariableDialog(wx.TextEntryDialog):
    ...
    def __init__(self, parent, iec_type, defaultValue=""):
        ...
        if self.IEC_Type == "BOOL":
            ...
            self.GetSizer().Insert(index=2, window=self.ToggleButton, border=border, flag=wx.LEFT | wx.RIGHT | wx.EXPAND)
    ...
    def ToggleBoolValue(self, event):
        ...
        self.ValueTextCtrl.SetValue(str(value))

    def OnOK(self, event):
        press_btn = event.GetId()
        if press_btn == wx.ID_OK:
            message = None
