...
class PLCControler:
    ...
    def GetProjectPouVariableTypes(self, pou_name=None, debug=False):
        variables = []
        project = self.GetProject(debug)
        if project is not None:
            for pou in project.getpous():
                if pou_name is None or pou_name == pou.getname():
                    variables.extend([var.Type for var in self.GetPouInterfaceVars(pou, debug=debug)])
                    for transition in pou.gettransitionList():
                        variables.append(transition.getname())
                    for action in pou.getactionList():
                        variables.append(action.getname())
    ...
    def GetProjectInfosFull(self, debug=False):
        ...
        if project is not None:
            ...
            if GlobalVariables: #len(GlobalVariables) > 0:
                InfoGlobVar = {"name": _("Global Variables"), "type": ITEM_CONFIGURATIONS, "values": [], "index_var": int}
                for GlobalVar in GlobalVariables:
                    InfoGlobVar["values"].append({"name": GlobalVar.Name + ", " + GlobalVar.Type, "type": ITEM_VARIABLE, "values": [], "index_var": GlobalVar.Number})
            ...
            for pou in project.getpous():
                ...
                pou_infos = {"name": pou.getname(), "type": ITEM_POU,
                             ...
                             "notPage": int}
                pou_values = []
                pou_variables = natsorted(self.GetEditedElementInterfaceVars("P::" + pou.getname()), key=lambda x: x.Name.lower())
                ...
                for resource in config.getresource():
                    ...
                    if resources_variables: #len(resources_variables) > 0:
                        ...
                        for ResurceVar in resources_variables:
                            resource_infos["values"].append({"name": ResurceVar.Name + ", " + ResurceVar.Type, "type": ITEM_VAR_GLOBAL, "values": [], "index_var": ResurceVar.Number})
        ...
        if self.Project is not None:
            try:
                ...
                if filepath is not None:
                    ...
                    programfile.write(str(program_text))
                    programfile.close()
    ...
    def SetGroupVariables(self, name, vars):
        #if self.Project is not None:
        pass
    ...
    # Return configuration variable type names
    def GetConfigurationVariableTypeNames(self, config_name=None, debug=False):
        variables = []
        configuration = None
        project = self.GetProject(debug)
        if project is not None:
            for configuration in self.Project.getconfigurations():
                for vars in configuration.getglobalVars():
                    for names in vars.getvariable():
                        variables.extend(names.tag)
        return variables
    ...
    def GetPouInterfaceReturnType(self, pou, tree=False, debug=False):
        ...
        if pou.interface is not None:
            ...
            if return_type is not None:
                ...
                arrClass = ["SetType", "AddDimension", "AddTree", "AddVarToTree"]  # Берем список тек функций которые нам нужны
                exten ={("var_infos_ns", name): getattr(factory, name)  for name in arrClass}
                paths = os.path.join(ScriptDirectory, "plcopen", "variables_infos.xslt")
    ...
    def GetEditedResourceVariables(self, tagname, debug=False):
        ...
        varlist = [var.Name for var in self.GetConfigurationResourceGlobalVars(words[1], words[2], debug) if var.Type == "BOOL"]
    ...
    def SetEditedFastTask(self, tagname, fasttasks):
        ...
        if resource is not None:
            resource.setfasttask([])
            resource.setevent([])
            task_list = {}
            for task in fasttasks:
                new_task = PLCOpenParser.CreateElement("fasttask", "resource")
                resource.appendfasttask(new_task)
                new_task.setname(task["Name"])
                new_task.setpriority(int(task["Priority"]))
                for event, mode in zip(task["Event"].split("\n"), task["Mode"].split("\n")):
                    new_event = PLCOpenParser.CreateElement("event", "fasttask")
                    new_task.appendevent(new_event)
                    new_event.setvalue(event)
                    new_event.setmode(mode if mode != " " else "")
                if task["Name"] != "":
                    task_list[task["Name"]] = new_task

    def SetEditedFastTaskDialog(self, tagname, fasttasks):
        resource = self.GetEditedElement(tagname)
        if resource is not None:
            #resource.settask([])
            ...
            for task in fasttasks:
                ...
                new_task.setname(task["Name"])
                new_task.setpriority(int(task["Priority"]))
                for event in task["Event"].split("\n"):
                    new_event = PLCOpenParser.CreateElement("event", "fasttask")
                    new_task.appendevent(new_event)
                    new_event.setvalue(event)
                    new_event.setmode("Counter")
                if task["Name"] != "":
                    task_list[task["Name"]] = new_task

    def SetEditedResourceInfos(self, tagname, tasks, instances):
        #self.SetEditedFastTask(tagname, tasks)
        resource = self.GetEditedElement(tagname)
        if resource is not None:
            resource.settask([])
    ...
    def GetEditedFastTask(self, tagname, debug=False):
        resource = self.GetEditedElement(tagname, debug)
        if resource is not None:
            fasttasks = resource.getfasttask()
            fasttasks_data = []
            for fasttask in fasttasks:
                events = fasttask.getevent()
                new_task = {}
                modes = []
                values = []
                new_task["Name"] = fasttask.getname()
                for event in events:
                    values.append(event.getvalue())
                    mode = event.getmode()
                    if mode is not None:
                        if mode == "":
                            mode = " "
                        ...
                        modes.append(" ")
                new_task["Mode"] = "\n".join(modes)
                    #new_task["Mode"] = "\n".join([event.getmode() for event in events])
                new_task["Event"] = "\n".join(values)
                new_task["Priority"] = str(fasttask.getpriority())
                fasttasks_data.append(new_task)
            return fasttasks_data

    def GetEditedFastTaskByName(self, tagname, name, debug=False):
        resource = self.GetEditedElement(tagname, debug)
        values = []
        modes = []
        if resource is not None:
            ...
            for fasttask in fasttasks:
                events = fasttask.getevent()
                task = {}
                fast_task_name = fasttask.getname()
                if fast_task_name == name:
                    task["Name"] = fast_task_name
                    for event in events:
                        values.append(event.getvalue())
                        mode = event.getmode()
                        if mode is not None:
                    ...
                    task["Mode"] = "\n".join(modes)
                    task["Event"] = "\n".join(values)
                    task["Priority"] = str(fasttask.getpriority())
                    return task

    def GetEditedFastTaskList(self, resource, debug=False):
        #resource = self.GetEditedElement(tagname, debug)
        if resource is not None:
            fasttasks = resource.getfasttask()
            return [fasttask.getname() for fasttask in fasttasks]
        return []
