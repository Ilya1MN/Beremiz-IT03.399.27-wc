...
class Translator:
    ...
    def __init__(self, mn, module_name, raw_module_name, src, debug, mod, output,
        ...
        if module_name.find(".") >= 0:
            ...
            vdec = 'var '
        print(UU+"%s%s = function (__mod_name__) {" % (vdec, module_name), file=self.output)

        print("    if("+module_name+".__was_initialized__) return;", file=self.output)
        print("    "+UU+module_name+".__was_initialized__ = true;", file=self.output)
        print(UU+"if (__mod_name__ == null) __mod_name__ = '%s';" % (mn), file=self.output)
        print(UU+"%s.__name__ = __mod_name__;" % (raw_module_name), file=self.output)
        ...
        if self.debug:
            ...
            print(haltException + ' = function () {', file=self.output)
            print('  this.message = "Program Halted";', file=self.output)
            print('  this.name = "' + haltException + '";', file=self.output)
            print('}', file=self.output)
            print('', file=self.output)
            print(haltException + ".prototype.__str__ = function()", file=self.output)
            print('{', file=self.output)
            print('return this.message ;', file=self.output)
            print('}', file=self.output)

            print(haltException + ".prototype.toString = function()", file=self.output)
            print('{', file=self.output)
            print('return this.name + ": \\"" + this.message + "\\"";', file=self.output)
            print('}', file=self.output)
    ...
    %s = function (s) {
      ...
      if (s.length < suffix.length) {
        ...
        return ss == suffix;
                ...
                """ % isHaltFunction, file=self.output)
    ...
    def add_imported_module(self, importName):
        ...
        print(gen_mod_import(self.raw_module_name,
                                             strip_py(importName),
                                             self.dynamic), file=self.output)
    ...
    def _default_args_handler(self, node, arg_names, current_klass,
        ...
        if len(node.defaults):
            ...
            for default_node in node.defaults:
                ...
                default_pos += 1
                print("    if (typeof %s == 'undefined') %s=%s;" % (default_name, default_name, default_value), file=output)

    def _varargs_handler(self, node, varargname, arg_names, current_klass):
        print("    var", varargname, '= new pyjslib.Tuple();', file=self.output)
        print("    for(var __va_arg="+str(len(arg_names))+"; __va_arg < arguments.length; __va_arg++) {", file=self.output)
        print("        var __arg = arguments[__va_arg];", file=self.output)
        print("        "+varargname+".append(__arg);", file=self.output)
        print("    }", file=self.output)
    ...
    def _kwargs_parser(self, node, function_name, arg_names, current_klass):
        if len(node.defaults) or node.kwargs:
            ...
            if arg_names and arg_names[0] == self.method_self:
                default_pos -= 1
            print(function_name+'.parse_kwargs = function (', ", ".join(["__kwargs"]+arg_names), ") {", file=self.output)
            for default_node in node.defaults:
...
#                    raise TranslationError("unsupported type (in _method)", default_node)
                ...
                default_name = arg_names[default_pos]
                print("    if (typeof %s == 'undefined')" % (default_name), file=self.output)
                print("        %s=__kwargs.%s;" % (default_name, default_name), file=self.output)
                default_pos += 1

            # self._default_args_handler(node, arg_names, current_klass)
            if node.kwargs:
                arg_names += ["pyjslib.Dict(__kwargs)"]
            print("    var __r = "+"".join(["[", ", ".join(arg_names), "]"])+";", file=self.output)
            if node.varargs:
                self._varargs_handler(node, "__args", arg_names, current_klass)
                print("    __r.push.apply(__r, __args.getArray())", file=self.output)
            print("    return __r;", file=self.output)
            print("};", file=self.output)
    ...
    def _function(self, node, local=False):
        ...
        print("%s = function%s {" % (function_name, function_args), file=self.output)
        ...
        if not isinstance(lastStmt, ast.Return):
            if not self._isNativeFunc(lastStmt):
                print("    return null;", file=self.output)

        print("};", file=self.output)
        print("%s.__name__ = '%s';\n" % (function_name, node.name), file=self.output)
    ...
    def _return(self, node, current_klass):
        ...
        # here too
        print("    return " + expr + ";", file=self.output)

    def _break(self, node, current_klass):
        print("    break;", file=self.output)

    def _continue(self, node, current_klass):
        print("    continue;", file=self.output)
    ...
    def _print(self, node, current_klass):
        ...
        print("pyjslib.printFunc([", ', '.join(call_args), "],", int(isinstance(node, ast.Printnl)), ");", file=self.output)
    ...
    def _tryExcept(self, node, current_klass):
        ...
        print("    try {", file=self.output)
        for stmt in node.body.nodes:
            self._stmt(stmt, current_klass)
        print("    } catch(%s) {" % errName, file=self.output)
        if expr:
            k = []
        ...
        if node.else_ is not None:
            print("    } finally {", file=self.output)
    ...
    def _class(self, node):
        ...
        print(UU+class_name_ + " = function () {", file=self.output)
        ...
        print(UU+class_name_ + ".__initialize__ = function () {", file=self.output)
        print("    if("+UU+class_name_+".__was_initialized__) return;", file=self.output)
        print("    "+UU+class_name_+".__was_initialized__ = true;", file=self.output)
        cls_obj = UU+class_name_ + '.prototype.__class__'

        if class_name == "pyjslib.__Object":
            print("    "+cls_obj+" = {};", file=self.output)
        else:
            if base_class and base_class not in ("object", "pyjslib.__Object"):
                print("    if(!"+UU+base_class_+".__was_initialized__)", file=self.output)
                print("        "+UU+base_class_+".__initialize__();", file=self.output)
                print("    pyjs_extend(" + UU+class_name_ + ", "+UU+base_class_+");", file=self.output)
            else:
                print("    pyjs_extend(" + UU+class_name_ + ", "+UU+"pyjslib.__Object);", file=self.output)

        print("    "+cls_obj+".__new__ = "+UU+class_name+";", file=self.output)
        print("    "+cls_obj+".__name__ = '"+UU+node.name+"';", file=self.output)
        ...
        print(class_name_+".__initialize__();", file=self.output)
    ...
    def _raise(self, node, current_klass):
        if node.expr2:
            raise TranslationError("More than one expression unsupported",
                                   node)
        print("throw (%s);" % self.expr(
            node.expr1, current_klass), file=self.output)
    ...
    def _method(self, node, current_klass, class_name, class_name_):
        ...
        if staticmethod:
            ...
            print("    " + UU+class_name_ + ".prototype.__class__." + node.name + " = " + class_name_+"."+node.name+";", file=self.output)
            print("    " + UU+class_name_ + ".prototype.__class__." + node.name + ".static_method = true;", file=self.output)
            return
        else:
            if len(arg_names) == 0:
        ...
        print("    "+fexpr + " = function" + function_args + " {", file=self.output)
        ...
        if classmethod:
            ...
            print("    "+altexpr + " = function() {", file=self.output)
            print("        return " + fexpr + ".apply(this.__class__, arguments);", file=self.output)
            print("    };", file=self.output)
            print("    "+fexpr+".class_method = true;", file=self.output)
            print("    "+altexpr+".instance_method = true;", file=self.output)
        else:
            # For instance methods, we need an unbound version in the class object
            altexpr = UU + class_name_ + ".prototype.__class__." + node.name
            print("    "+altexpr + " = function() {", file=self.output)
            print("        return " + fexpr + ".call.apply("+fexpr+", arguments);", file=self.output)
            print("    };", file=self.output)
            print("    "+altexpr+".unbound_method = true;", file=self.output)
            print("    "+fexpr+".instance_method = true;", file=self.output)
            print("    "+altexpr+".__name__ = '%s';" % node.name, file=self.output)

        print(UU + class_name_ + ".prototype.%s.__name__ = '%s';" % \
            (node.name, node.name), file=self.output)

        if node.kwargs or len(node.defaults):
            print("    "+altexpr + ".parse_kwargs = " + fexpr + ".parse_kwargs;", file=self.output)
    ...
    def _augassign(self, node, current_klass):
        ...
        print("    " + lhs + " " + op + " " + rhs + ";", file=self.output)
    ...
    def _assign(self, node, current_klass, top_level=False):
        ...
        if isinstance(v, ast.AssAttr):
            ...
            if v.flags == "OP_ASSIGN":
                ...
                print("    " + obj + ".__setitem__(" + idx + ", " + value + ");", file=self.output)
                return
            else:
                raise TranslationError("unsupported flag (in _assign)", v)
            ...
            print("    var " + tempName + " = " + \
                                 self.expr(node.expr, current_klass) + ";", file=self.output)
            for index, child in enumerate(v.getChildNodes()):
                ...
                if isinstance(child, ast.AssAttr):
                    ...
                    if child.flags == "OP_ASSIGN":
                        ...
                        value = self.expr(node.expr, current_klass)
                        print("    " + obj + ".__setitem__(" \
                            + idx + ", " + rhs + ");", file=self.output)
                        continue
                print("    " + lhs + " = " + rhs + ";", file=self.output)
            return
        else:
    ...
    def _discard(self, node, current_klass):
        ...
        if isinstance(node.expr, ast.CallFunc):
            ...
            if isinstance(node.expr.node, ast.Name) and node.expr.node.name == NATIVE_JS_FUNC_NAME:
                ...
                print(raw_js, file=self.output)
            else:
                expr = self._callfunc(node.expr, current_klass)
                print("    " + expr + ";", file=self.output)
            ...
            if node.expr.value is not None:  # Empty statements generate ignore None
                print(self._const(node.expr), file=self.output)
        else:
    ...
    def _if_test(self, keyword, test, consequence, current_klass):
        if test:
            ...
            print("    " + keyword + " (pyjslib.bool(" + expr + ")) {", file=self.output)
        else:
            print("    " + keyword + " {", file=self.output)
    ...
    def _while(self, node, current_klass):
        ...
        print("    while (pyjslib.bool(" + test + ")) {", file=self.output)
    ...
    def _subscript_stmt(self, node, current_klass):
        if node.flags == "OP_DELETE":
            print("    " + self.expr(node.expr, current_klass) + ".__delitem__(" + self.expr(node.subs[0], current_klass) + ");", file=self.output)
        else:
            raise TranslationError("unsupported flag (in _subscript)", node)
