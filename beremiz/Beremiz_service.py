    ...
    if havewx:
        ...
        class BeremizTaskBarIcon(wx.adv.TaskBarIcon):
            ...
            def OnTaskBarStartPLC(self, evt):
                if self.pyroserver.plcobj is not None:
                    ...
                    if plcstatus == "Stopped":
                        ...
                        print(("PLC is empty or already started."))
            ...
            def OnTaskBarStopPLC(self, evt):
                if self.pyroserver.plcobj is not None:
                    if self.pyroserver.plcobj.GetPLCstatus()[0] == "Started":
                        ...
                        print(("PLC is not started."))

            def OnTaskBarChangeInterface(self, evt):
                ip_addr = self.pyroserver.ip_addr
                ip_addr = '' if ip_addr is None else ip_addr
                dlg = ParamsEntryDialog(None, ("Enter the IP of the interface to bind"), defaultValue=ip_addr)
            ...
            def OnTaskBarChangeName(self, evt):
                ...
                dlg = ParamsEntryDialog(None, ("Enter a name "), defaultValue=servicename)
                dlg.SetTests([(lambda name: len(name) != 0, ("Name must not be null!"))])
                if dlg.ShowModal() == wx.ID_OK:
            ...
            def UpdateIcon(self, plcstatus):
                if plcstatus == "Started":
                    currenticon = self.MakeIcon(starticon)
                elif plcstatus == "Stopped":
...
class Server(object):
    ...
    def Start(self):
        self.daemon = Pyro4.Daemon(host=self.ip_addr, port=self.port)
        self.plcobj = PLCObject(self.workdir, self.daemon, self.argv,
                                self.statuschange, self.evaluator,
                                self.pyruntimevars)

        self.daemon.register(obj_or_class=self.plcobj, objectId="PLCObject")
        uri = self.daemon.uriFor(self.plcobj)
...
for extfilename in extensions:
    ...
    exec(compile(open(extfilename, "rb").read(), extfilename, 'exec'), locals())
