...
PLCOpen_v1_xsd = etree.XMLSchema(etree.fromstring(PLCOpen_v1_xml.encode('ascii')))
...
def LoadProject(filepath):
    ...
    try:
        ...
        project_file.close()
        project_file = open(filepath, encoding='cp1251')
        project_xml = project_file.read()
        pass
    project_file.close()
...
if cls:
    ...
    def getpous(self, exclude=None, filter=[]):
        str1 = "ppx:types/ppx:pous/ppx:pou%s%s"
        str21 = ("[%s]" % " or ".join(["@pouType='%s'" % x for x in filter]))
        str2 = (("[@name!='%s']" % exclude) if exclude is not None else '',
                                                str21
                                                if filter else "")
        return self.xpath( str1 % str2,
