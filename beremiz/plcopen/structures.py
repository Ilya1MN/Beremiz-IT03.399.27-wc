...
def GetSubTypes(type):
    ...
    """
    return [typename for typename, parenttype in list(TypeHierarchy.items()) if not typename.startswith("ANY") and IsOfType(typename, type)]
...
def csv_file_to_table(file):
    ...
    """
    return [list(map(str.strip, line.split(';'))) for line in file]
    ...
    for fields in table:
        if fields[1]:
            ...
            if Current_section:
                ...
                raise str("First function must be in a category")
