...
from IDEFrame import TITLE, FILEMENU, PROJECTTREE, PAGETITLES, VARIABLETREE

from controls import TextCtrlAutoComplete
from dialogs import BrowseValuesLibraryDialog
from util.BitmapLibrary import GetBitmap
from connectors import connectors
from runtime.SerialPortList import GetSerialPorts
...
class ConfTreeNodeEditor(EditorPanel):
    ...
    def GenerateSizerElements(self, sizer, elements, path, clean=True):
        ...
        for element_infos in elements:
            ...
            if element_infos["type"] == "element":
                ...
                boxsizer = wx.FlexGridSizer(cols=3, rows=1, hgap=0, vgap=0)
                ...
                if isinstance(element_infos["type"], list):
                    ...
                    if element_infos["type"] == "boolean":
                        ...
                        if element_path != 'BeremizRoot.URI_location':
                            ...
                            hostname = ""
                            try:
                                hostname, com_port = element_infos["value"].split("://")
                            except:
                                pass
                            combobox = self.GenerateCombobox(connectors.keys(), element_infos, sizer, "Server", hostname)
                            combobox.Bind(wx.EVT_COMBOBOX, self.EvtServerChange, combobox)

                            combobox = self.GenerateCombobox(GetSerialPorts(), element_infos, sizer, "COM", com_port)
                            combobox.Bind(wx.EVT_COMBOBOX, self.EvtCOM_Change, combobox)
                            '''combobox = wx.ComboBox(self.ParamsEditor,
                                                   size=wx.Size(300, -1), style=wx.CB_READONLY)
                            boxsizer.Add(combobox)
                            combobox.Append("")
                            for ports in GetSerialPorts():
                                combobox.Append(ports)'''

            first = False
        sizer.Layout()
        self.RefreshScrollbars()

    def GenerateCombobox(self, keys, element_infos, sizer, name, value):
        boxsizer = wx.FlexGridSizer(cols=3, rows=1, hgap=0, vgap=0)
        boxsizer.AddGrowableCol(1)
        flags = (wx.GROW | wx.BOTTOM | wx.LEFT | wx.RIGHT)
        sizer.Add(boxsizer, border=5, flag=flags)
        ...
        for key in keys:
            combobox.Append(key)
        str_cnt = combobox.FindString(value)
        if str_cnt is not wx.NOT_FOUND:
            combobox.SetValue(combobox.GetString(str_cnt))
        return combobox
    ...
    def GetItemChannelChangedFunction(self, dir):
        def OnConfNodeTreeItemChannelChanged(event):
            ...
            wx.CallAfter(self.ParentWindow._Refresh, TITLE, FILEMENU, PROJECTTREE, VARIABLETREE)
            event.Skip()
    ...
    def GetTextCtrlCallBackFunction(self, textctrl, path, refresh=False):
        def OnTextCtrlChanged(event):
            ...
            if refresh:
                wx.CallAfter(self.ParentWindow._Refresh, TITLE, FILEMENU, PROJECTTREE, PAGETITLES, VARIABLETREE)
    ...
    def EvtServerChange(self, event):
        pass

    def EvtCOM_Change(self, event):
        pass

    def GetUriServerCallBackFunction(self, textctrl, path, old_value, refresh=False):
        def OnUriChanged(event):
    ...
    def GetUriPortCallBackFunction(self, textctrl, path, old_value, refresh=False):
        def OnUriChanged(event):
