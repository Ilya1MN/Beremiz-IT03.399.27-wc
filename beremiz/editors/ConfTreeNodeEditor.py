                ...
                if isinstance(element_infos["type"], list):
                    ...
                    if element_infos["type"] == "boolean":
                        ...
                        if element_path == 'BeremizRoot.URI_location':
                            if "YAPLC" in connectors.keys():
                                choices = ""#["YAPLC://%s" % com_port for com_port in GetSerialPorts()]
                                refresh = True
                        textctrl = TextCtrlAutoComplete(name=element_infos["name"],
                                                        parent=self.ParamsEditor,
                                                        choices=choices,
                                                        element_path=element_path,
                                                        size=wx.Size(300, -1), refresh_list=GetSerialPorts)

                        boxsizer.Add(textctrl)
                        if element_infos["value"] is not None:
                            textctrl.ChangeValue(str(element_infos["value"]))
                        callback = self.GetTextCtrlCallBackFunction(textctrl, element_path, refresh=refresh)
