...
class Viewer(EditorPanel, DebugViewer):
    ...
    def AddBlockPinMenuItems(self, menu, connector):
        [ID_NO_MODIFIER, ID_NEGATED, ID_RISING_EDGE,
         ID_FALLING_EDGE] = [wx.NewId() for i in range(4)]

        # Create menu items
        self.AddMenuItems(menu, [
            (ID_NO_MODIFIER, wx.ITEM_RADIO, _('No Modifier'), '', self.OnNoModifierMenu),
            (ID_NEGATED, wx.ITEM_RADIO, _('Negated'), '', self.OnNegatedMenu),
            (ID_RISING_EDGE, wx.ITEM_RADIO, _('Rising Edge'), '', self.OnRisingEdgeMenu),
            (ID_FALLING_EDGE, wx.ITEM_RADIO, _('Falling Edge'), '', self.OnFallingEdgeMenu)])
    ...
    def AddAlignmentMenuItems(self, menu):
        ...
        # Create menu items
        self.AddMenuItems(menu, [
            (ID_ALIGN_LEFT, wx.ITEM_NORMAL, _('Left'), '', self.OnAlignLeftMenu),
            (ID_ALIGN_CENTER, wx.ITEM_NORMAL, _('Center'), '', self.OnAlignCenterMenu),
            (ID_ALIGN_RIGHT, wx.ITEM_NORMAL, _('Right'), '', self.OnAlignRightMenu),
            None,
            (ID_ALIGN_TOP, wx.ITEM_NORMAL, _('Top'), '', self.OnAlignTopMenu),
            (ID_ALIGN_MIDDLE, wx.ITEM_NORMAL, _('Middle'), '', self.OnAlignMiddleMenu),
            (ID_ALIGN_BOTTOM, wx.ITEM_NORMAL, _('Bottom'), '', self.OnAlignBottomMenu)])
    ...
    def AddWireMenuItems(self, menu, delete=False, replace=False):
        ...
        # Create menu items
        self.AddMenuItems(menu, [
            (ID_ADD_SEGMENT, wx.ITEM_NORMAL, _('Add Wire Segment'), '', self.OnAddSegmentMenu),
            (ID_DELETE_SEGMENT, wx.ITEM_NORMAL, _('Delete Wire Segment'), '', self.OnDeleteSegmentMenu),
            (ID_REPLACE_WIRE, wx.ITEM_NORMAL, _('Replace Wire by connections'), '', self.OnReplaceWireMenu)])
    ...
    def AddDivergenceMenuItems(self, menu, delete=False):
        [ID_ADD_BRANCH, ID_DELETE_BRANCH] = [wx.NewId() for i in range(2)]

        # Create menu items
        self.AddMenuItems(menu, [
            (ID_ADD_BRANCH, wx.ITEM_NORMAL, _('Add Divergence Branch'), '', self.OnAddBranchMenu),
            (ID_DELETE_BRANCH, wx.ITEM_NORMAL, _('Delete Divergence Branch'), '', self.OnDeleteBranchMenu)])
    ...
    def AddAddMenuItems(self, menu):
        [ID_ADD_BLOCK, ID_ADD_VARIABLE, ID_ADD_CONNECTION,
         ID_ADD_COMMENT] = [wx.NewId() for i in range(4)]

        # Create menu items
        self.AddMenuItems(menu, [
            (ID_ADD_BLOCK, wx.ITEM_NORMAL, _('Block'), '', self.GetAddMenuCallBack(self.AddNewBlock)),
            (ID_ADD_VARIABLE, wx.ITEM_NORMAL, _('Variable'), '', self.GetAddMenuCallBack(self.AddNewVariable)),
            (ID_ADD_CONNECTION, wx.ITEM_NORMAL, _('Connection'), '', self.GetAddMenuCallBack(self.AddNewConnection)),
            None])
        ...
        if self.CurrentLanguage != "FBD":
            ...
            # Create menu items
            self.AddMenuItems(menu, [
                (ID_ADD_POWER_RAIL, wx.ITEM_NORMAL, _('Power Rail'), '', self.GetAddMenuCallBack(self.AddNewPowerRail)),
                (ID_ADD_CONTACT, wx.ITEM_NORMAL, _('Contact'), '', self.GetAddMenuCallBack(self.AddNewContact))])

            if self.CurrentLanguage != "SFC":
                self.AddMenuItems(menu, [
                     (ID_ADD_COIL, wx.ITEM_NORMAL, _('Coil'), '', self.GetAddMenuCallBack(self.AddNewCoil))])
        ...
        if self.CurrentLanguage == "SFC":
            ...
            # Create menu items
            self.AddMenuItems(menu, [
                (ID_ADD_INITIAL_STEP, wx.ITEM_NORMAL, _('Initial Step'), '', self.GetAddMenuCallBack(self.AddNewStep, True)),
                (ID_ADD_STEP, wx.ITEM_NORMAL, _('Step'), '', self.GetAddMenuCallBack(self.AddNewStep)),
                (ID_ADD_TRANSITION, wx.ITEM_NORMAL, _('Transition'), '', self.GetAddMenuCallBack(self.AddNewTransition)),
                (ID_ADD_ACTION_BLOCK, wx.ITEM_NORMAL, _('Action Block'), '', self.GetAddMenuCallBack(self.AddNewActionBlock)),
                (ID_ADD_DIVERGENCE, wx.ITEM_NORMAL, _('Divergence'), '', self.GetAddMenuCallBack(self.AddNewDivergence)),
                (ID_ADD_JUMP, wx.ITEM_NORMAL, _('Jump'), '', self.GetAddMenuCallBack(self.AddNewJump)),
                None])

        self.AddMenuItems(menu, [
             (ID_ADD_COMMENT, wx.ITEM_NORMAL, _('Comment'), '', self.GetAddMenuCallBack(self.AddNewComment))])

    # Add Default Menu items to the given menu
    def AddDefaultMenuItems(self, menu, edit=False, block=False):
        if block:
            [ID_EDIT_BLOCK, ID_DELETE, ID_ADJUST_BLOCK_SIZE] = [wx.NewId() for i in range(3)]

            # Create menu items
            self.AddMenuItems(menu, [
                 (ID_EDIT_BLOCK, wx.ITEM_NORMAL, _('Edit Block'), '', self.OnEditBlockMenu),
                 (ID_ADJUST_BLOCK_SIZE, wx.ITEM_NORMAL, _('Adjust Block Size'), '', self.OnAdjustBlockSizeMenu),
                 (ID_DELETE, wx.ITEM_NORMAL, _('Delete'), '', self.OnDeleteMenu)])

            menu.Enable(ID_EDIT_BLOCK, edit)

        else:
            [ID_CLEAR_EXEC_ORDER, ID_RESET_EXEC_ORDER] = [wx.NewId() for i in range(2)]

            # Create menu items
            if self.CurrentLanguage == 'FBD':
                self.AddMenuItems(menu, [
                    (ID_CLEAR_EXEC_ORDER, wx.ITEM_NORMAL, _('Clear Execution Order'), '', self.OnClearExecutionOrderMenu),
                    (ID_RESET_EXEC_ORDER, wx.ITEM_NORMAL, _('Reset Execution Order'), '', self.OnResetExecutionOrderMenu)])
                menu.AppendSeparator()

            add_menu = wx.Menu(title='')
            self.AddAddMenuItems(add_menu)

            menu.Append(-1, _('Add'), add_menu)

        menu.AppendSeparator()

        [ID_CUT, ID_COPY, ID_PASTE] = [wx.NewId() for i in range(3)]

        # Create menu items
        self.AddMenuItems(menu, [
             (ID_CUT, wx.ITEM_NORMAL, _('Cut'), '', self.GetClipboardCallBack(self.Cut)),
             (ID_COPY, wx.ITEM_NORMAL, _('Copy'), '', self.GetClipboardCallBack(self.Copy)),
             (ID_PASTE, wx.ITEM_NORMAL, _('Paste'), '', self.GetAddMenuCallBack(self.Paste))])
        ...
        if self.CurrentScale != new_scale:
            ...
            if refresh:
                ...
                if mouse_event is None:
                    ...
                    mouse_pos = wx.Point(int(client_size[0] / 2)), int(client_size[1] / 2)
                ...
                scrollx = max(0, int(round(pos.x * self.ViewScale[0] - mouse_pos.x) / SCROLLBAR_UNIT))
                scrolly = max(0, int(round(pos.y * self.ViewScale[1] - mouse_pos.y) / SCROLLBAR_UNIT))
    ...
    def RefreshScrollBars(self, width_incr=0, height_incr=0):
        ...
        maxx = (maxx * self.ViewScale[0])
        maxy = (maxy * self.ViewScale[1])
    ...
    def EnsureVisible(self, block):
        ...
        if block_minx < screen_minx and block_maxx < screen_maxx:
            xpos -= int((screen_minx - block_minx) / SCROLLBAR_UNIT + 1)
        elif block_maxx > screen_maxx and block_minx > screen_minx:
            xpos += int((block_maxx - screen_maxx) / SCROLLBAR_UNIT + 1)
        if block_miny < screen_miny and block_maxy < screen_maxy:
            ypos -= int((screen_miny - block_miny) / SCROLLBAR_UNIT + 1)
        elif block_maxy > screen_maxy and block_miny > screen_miny:
            ypos += int((block_maxy - screen_maxy) / SCROLLBAR_UNIT + 1)
        ...
        if self.Mode == MODE_SELECTION:
            ...
            if event.ShiftDown() and not event.ControlDown() and self.SelectedElement is not None:
                ...
                if not self.Debug and self.DrawingWire:
                    ...
                    wire = Wire(self, *list(map(list, list(zip(
    ...
    def OnMouseWheelWindow(self, event):
        if self.StartMousePos is None or self.StartScreenPos is None:
            rotation = int(event.GetWheelRotation() / event.GetWheelDelta())
            if event.ShiftDown():
                x, y = self.GetViewStart()
    ...
    def DoDrawing(self, dc, printing=False):
        if printing:
            ...
            dc.SetBackground(wx.Brush(self.Editor.GetBackgroundColour()))
            dc.Clear()
            box = dc.GetClippingBox()
            dc.DrawBitmap(dc.GetSelectedBitmap(), box[1], box[2])
