...
HIGHLIGHT_TYPES = {(ERROR_HIGHLIGHT[0].GetIM(), ERROR_HIGHLIGHT[1].GetIM()): STC_PLC_ERROR, (SEARCH_RESULT_HIGHLIGHT[0].GetIM(), SEARCH_RESULT_HIGHLIGHT[1].GetIM()): STC_PLC_SEARCH_RESULT,}


def LineStartswith(line, symbols):
    return reduce(lambda x, y: x or y, [line.startswith(x) for x in symbols], False)
...
class TextViewer(EditorPanel):
    ...
    def RefreshLineFolding(self, line_number):
        if self.TextSyntax in ["ST", "ALL"]:
            ...
            if line == "":
                if line_number > 0:
                    if LineStartswith(self.Editor.GetLine(line_number - 1).strip().upper(), self.BlockEndKeywords):
            ...
            elif LineStartswith(line.upper(), self.BlockStartKeywords):
                level |= wx.stc.STC_FOLDLEVELHEADERFLAG
            elif LineStartswith(line.upper(), self.BlockEndKeywords):
                if LineStartswith(self.Editor.GetLine(line_number - 1).strip().upper(), self.BlockEndKeywords):
                    level = self.Editor.GetFoldLevel(self.Editor.GetFoldParent(line_number - 1)) & wx.stc.STC_FOLDLEVELNUMBERMASK
                else:
                    level = self.Editor.GetFoldLevel(line_number - 1) & wx.stc.STC_FOLDLEVELNUMBERMASK
        ...
        if self.Controler is not None:
            ...
            if key == wx.WXK_SPACE and event.ControlDown():
                ...
                if self.TextSyntax in ["ST", "ALL"]:
                    ...
                    if LineStartswith(lineText.strip().upper(), self.BlockStartKeywords):
                        indent = int((indent / 2 + 1) * 2)
                    self.Editor.AddText("\n" + " " * indent)
                    key_handled = True
                ...
                if self.TextSyntax in ["ST", "ALL"]:
                    if not self.Editor.GetSelectedText():
                        ...
                        if lineText.strip() == "" and len(lineText) > 0 and indent > 0:
                            ...
                            var = int(((max(0, indent - 1) / 2) * 2))
                            self.Editor.AddText(" " * var)
                            key_handled = True
            if not key_handled:
                event.Skip()
        else:
    ...
    def ClearHighlights(self, highlight_type=None):
        highlight_type = (highlight_type[0].GetIM(), highlight_type[1].GetIM())
