...
def UnpackDebugBuffer(buff, indexes):
    res = []
        ...
        #buff = buff["data"].encode()
    buffoffset = 0
    if isinstance(buff, dict):
        if buff["encoding"] == "base64":
            _buff = base64.b64decode(buff["data"])
    else:
        _buff = buff
    buff_array = ctypes.create_string_buffer(_buff)
    buffsize = len(_buff)
    buffptr = cast(buff_array, c_void_p).value
    for iectype in indexes:
        c_type, unpack_func, pack_func = \
            TypeTranslator.get(iectype, (None, None, None))
        if c_type is not None and buffoffset < buffsize:
            cursor = c_void_p(buffptr + buffoffset)
            value = unpack_func(cast(cursor, POINTER(c_type)).contents)
