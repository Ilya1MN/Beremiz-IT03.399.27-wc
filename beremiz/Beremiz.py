...
class BeremizIDELauncher:
    ...
    def CreateApplication(self):
        if os.path.exists("BEREMIZ_DEBUG"):
            builtins.__dict__["BMZ_DBG"] = True
        else:
            builtins.__dict__["BMZ_DBG"] = False
        ...
        if wx.VERSION >= (3, 0, 0):
            now = datetime.now()
            #filename = ("mylogfile_%s.txt")%(now.strftime("%H_%M_%S"))
            #self.app = wx.App(redirect=True, filename=filename)
        ...
        if self.updateinfo_url is not None:
            ...
            def updateinfoproc():
                ...
                try:
                    ...
                    updateinfo = urllib.request.urlopen(self.updateinfo_url, None).read()
                except Exception:
