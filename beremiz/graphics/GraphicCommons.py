...
class Graphic_Element(ToolTipProducer):
    ...
    def FindNearestConnector(self, position, connectors):
        ...
        try:
            ...
            for i, dist in distances:
                if i not in pos_xy_list:
                    pos_xy_list.append(i)
                    dist_sort.append([i, dist])
            dist_sort.sort()
            distances = dist_sort
    ...
    def GetRedrawRect(self, movex=0, movey=0):
        ...
        rect = wx.Rect()
        rect.x = self.BoundingBox.x - (HANDLE_SIZE / scalex) - 3 - abs(movex)
        rect.y = self.BoundingBox.y - (HANDLE_SIZE / scaley) - 3 - abs(movey)
    ...
    def GetRedrawRect(self, movex=0, movey=0):
        ...
        if self.ValueSize is not None:
            ...
            if self.BoundingBox[2] > width * 4 or self.BoundingBox[3] > height * 4:
                x = self.Points[0].x + width * int(self.StartPoint[1][0] / 2)
                y = self.Points[0].y + height * (self.StartPoint[1][1] - 1)
                rect = rect.Union(wx.Rect(x, y, width, height))
                x = self.Points[-1].x + width * int(self.EndPoint[1][0] / 2)
                y = self.Points[-1].y + height * (self.EndPoint[1][1] - 1)
                rect = rect.Union(wx.Rect(x, y, width, height))
            else:
                middle = int(len(self.Segments) / 2) + int(len(self.Segments) % 2 - 1)
                ...
                if self.BoundingBox[3] > height and self.Segments[middle] in [NORTH, SOUTH]:
                    y = int((self.Points[middle].y + self.Points[middle + 1].y - height) / 2)
                else:
            ...
            if i + 1 == len(self.Points) - 1:
                ...
                # The current point is the second
                    ...
                    # perpendiculars and the end direction points towards current segment
                        ...
                        if self.Segments[0][0] != 0:
                            self.Points[1].x = int((end.x + start.x) / 2)
                        if self.Segments[0][1] != 0:
                            self.Points[1].y = int((end.y + start.y) / 2)
                        self.Points.insert(2, wx.Point(self.Points[1].x, self.Points[1].y))
                        self.Segments.insert(2, DirectionChoice(
                            (self.Segments[1][1],
                             self.Segments[1][0]), v_end, self.EndPoint[1]))
    ...
    def AddSegment(self):
        ...
        if handle_type == HANDLE_SEGMENT:
            ...
            if len(self.Segments) > 1:
                ...
                if dir[0] != 0:
                    pointx = int((self.Points[segment].x + self.Points[segment + 1].x) / 2)
                if dir[1] != 0:
                    pointy = int((self.Points[segment].y + self.Points[segment + 1].y) / 2)
                ...
                if dir[0] != 0:
                    p1x = int((2 * self.Points[segment].x + self.Points[segment + 1].x) / 3)
                    p2x = int((self.Points[segment].x + 2 * self.Points[segment + 1].x) / 3)
                if dir[1] != 0:
                    p1y = int((2 * self.Points[segment].y + self.Points[segment + 1].y) / 3)
                    p2y = int((self.Points[segment].y + 2 * self.Points[segment + 1].y) / 3)
    ...
    def OnLeftDClick(self, event, dc, scaling):
        ...
        if event.ControlDown():
            ...
            if direction in [(EAST, WEST), (WEST, EAST)]:
                avgy = int((self.StartPoint[0].y + self.EndPoint[0].y) / 2)
                if scaling is not None:
                ...
                avgx = int((self.StartPoint[0].x + self.EndPoint[0].x) / 2)
                if scaling is not None:
        ...
        if self.Value is not None and not isinstance(self.Value, bool) and self.Value != "undefined":
            ...
            if self.ValueSize is not None:
                ...
                if self.BoundingBox[2] > width * 4 or self.BoundingBox[3] > height * 4:
                    ...
                    middle = int(len(self.Segments) / 2) + int(len(self.Segments) % 2 - 1)
                    x = int((self.Points[middle].x + self.Points[middle + 1].x - width) / 2)
                    if self.BoundingBox[3] > height and self.Segments[middle] in [NORTH, SOUTH]:
                        y = int((self.Points[middle].y + self.Points[middle + 1].y - height) / 2)
                    else:
                        y = self.Points[middle].y - height
...
class Comment(Graphic_Element):
    ...
    def ClearHighlight(self, highlight_type=None):
        #TODO ERROR
        self.Highlights = CleaClearHighlights(self.Highlights, highlight_type)
