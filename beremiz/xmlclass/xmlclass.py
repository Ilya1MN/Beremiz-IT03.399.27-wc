        ...
        if infos["elmt_type"]["type"] == SIMPLETYPE:
            ...
            def initial_value():
                ...
                if infos["type"] != ANY:
                    ...
                    value._init_()
                return value
        return [initial_value() for i in range(infos["minOccurs"])]
    else:
        return []
...
def GenerateContentInfos(factory, name, choices):
    ...
    def GetContentInitial():
        ...
        if content_name == "sequence":
            content_value = []
            for i in range(infos["minOccurs"]):
...
class DefaultElementClass(etree.ElementBase):
    ...
    def tostring(self):
        #return NAMESPACE_PATTERN.sub("", etree.tostring(self, pretty_print=True, encoding='utf-8').decode('cp1251'))
