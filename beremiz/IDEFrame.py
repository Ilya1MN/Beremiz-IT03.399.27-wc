...
get_indexes = lambda x, searchable: [i for (y, i) in zip(searchable, range(len(searchable))) if
                                                 x == y.Name]
...
[
    ...
    ID_PLCOPENEDITORPROJECTPANEL, ID_PLCOPENEDITORVARIABLEPANEL, ID_PLCOPENEDITORVARIABLESEARCHCTRL,
    ID_PLCOPENEDITORVARIABLETREE, ID_PLCOPENEDITORVARIABLECOMMENT,
...
SORT_ITEM_TREE_LIST = [_("Global Variables"), _("Functions"), _("Function Blocks"),
                       _("Programs"), _("Resources")]
# -------------------------------------------------------------------------------
#                               Helper Functions
# -------------------------------------------------------------------------------
...
def DecodeFileSystemPath(path, is_base64=False):
    if is_base64:
        path = base64.b64decode(path)
    return str(path)#, str(sys.getfilesystemencoding())
...
class IDEFrame(wx.Frame):
    ...
    def _init_coll_EditMenu_Items(self, parent):
        ...
        enable_undo_redo = _('Enable Undo/Redo')  # Keeping text in translations for possible menu reactivation
    ...
    def _init_ctrls(self, prnt):
        ...
        MenuToolBar.Realize()
        MenuToolBar.SetBackgroundColour(wx.Colour("WHITE"))
        ...
        EditorToolBar.AddRadioTool(ID_PLCOPENEDITOREDITORTOOLBARSELECTION, 'selected', GetBitmap("select"), wx.NullBitmap, _("Select an object"))
        EditorToolBar.Realize()
        EditorToolBar.SetBackgroundColour(wx.Colour("WHITE"))
        ...
        # -----------------------------------------------------------------------
        #                            Creating Variable tree
        # -----------------------------------------------------------------------

        self.MainTabs = {}

        self.VariableTree = CT.CustomTreeCtrl(id=ID_PLCOPENEDITORVARIABLETREE,
                                      name='VariableTree',
                                      pos=wx.Point(0, 0), size=wx.Size(0, 0),
                                      style=wx.SUNKEN_BORDER,
                                      parent=self.RightNoteBook,
                                      agwStyle=(wx.TR_HAS_BUTTONS |
                                                wx.TR_SINGLE))

        self.VariableTree.SetBackgroundColour(wx.Colour("WHITE"))

        self.VariableTree.Bind(wx.EVT_TREE_ITEM_ACTIVATED, self.OnDClick)

        # -----------------------------------------------------------------------
        #        Creating PLCopen Project POU Instance Variables Panel
        # -----------------------------------------------------------------------

        self.MainTabs["VariableTree"] = (self.VariableTree, _("Variables"))
        self.RightNoteBook.AddPage(*self.MainTabs["VariableTree"])
        ...
        self.SearchResultPanel.SetBackgroundColour(wx.Colour("WHITE"))
    ...
    def __init__(self, parent, enable_debug=False):
        ...
        # self.TreeImageDict["Delete"] = self.TreeImageList.Add(GetBitmap("Delete")
        self.TreeImageDict["Delete"] = self.TreeImageList.Add(GetBitmap("Delete"))

        # Assign icon list to TreeCtrls
        self.ProjectTree.SetImageList(self.TreeImageList)
        self.VariableTree.SetImageList(self.TreeImageList)
    ...
    def GetTabInfos(self, tab):
        for page_name, (page_ref, page_title) in self.MainTabs.items():
            if page_ref == tab:
    ...
    def RestoreLastState(self):
        ...
        if self.Config.HasEntry("framesize"):
            config = self.Config.Read("framesize")
            VFrameSzIndex = config.find("I")
            HFrameSzIndex = config.find("I", config.find("I")+1)
            frame_size = []
            if VFrameSzIndex != -1 and HFrameSzIndex != -1:
                FrameSz = ""
                for index in range(VFrameSzIndex + 1, config.find("\n", VFrameSzIndex)):
                    FrameSz += config[index]
                frame_size.append(int(FrameSz))
                FrameSz = ""
                for index in range(HFrameSzIndex + 1, config.find("\n", HFrameSzIndex)):
                    FrameSz += config[index]
                frame_size.append(int(FrameSz))
            else:
                frame_size = config[1:len(config)-1].split(',')
                frame_size[0] = int(frame_size[0])
                frame_size[1] = int(frame_size[1])
    ...
    def SaveLastState(self):
        if not self.IsMaximized():
            a = self.GetClientSize()
            self.Config.Write("framesize", str(self.GetClientSize()))
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if new_values != old_values:
                ...
                self._Refresh(TITLE, EDITORTOOLBAR, FILEMENU, EDITMENU, DISPLAYMENU,
                              PROJECTTREE, POUINSTANCEVARIABLESPANEL, SCALING, VARIABLETREE)
        dialog.Destroy()
    ...
    def ResetView(self):
        ...
        self.VariableTree.DeleteAllItems()
        self.VariableTree.Enable(False)
    ...
    def OnUndoMenu(self, event):
        ...
        self._Refresh(TITLE, FILEMENU, EDITMENU, PROJECTTREE, POUINSTANCEVARIABLESPANEL, LIBRARYTREE,
                      SCALING, PAGETITLES, VARIABLETREE)
            ...
            if selected is not None and selected.IsOk():
                ...
                if function is not None:
                    ...
                    self._Refresh(TITLE, EDITORTOOLBAR, FILEMENU, EDITMENU, PROJECTTREE,
                                  POUINSTANCEVARIABLESPANEL, LIBRARYTREE, VARIABLETREE)
    ...
    def RefreshTabCtrlEvent(self):
        ...
        if self.TabsOpened.GetPageCount() == 0:
            ...
            self.AUIManager.RestoreMaximizedPane()
            self.AUIManager.Update()

    def EnsureTabVisible(self, tab):
        notebook = tab.GetParent()
        if notebook != self.RightNoteBook:
    ...
    def SwitchFullScrMode(self, evt):
        pane = self.AUIManager.GetPane(self.TabsOpened)
        self.IsMaxFlag = not self.IsMaxFlag
...
#20.04.2022  <- Дата предыдущего автора
    ...
    def OnProjectTreeRightUp(self, event):
        ...
        if item_infos["type"] in ITEMS_UNEDITABLE + [ITEM_PROJECT]:
            ...
            if item_infos["type"] == ITEM_POU:
                ...
                #new_id = wx.NewId()
                #AppendMenu(menu, help='', id=new_id, kind=wx.ITEM_NORMAL, text=_("Paste POU"))
                #elf.Bind(wx.EVT_MENU, self.OnPastePou, id=new_id)
                ...
                if pou_type in ["function", "functionBlock"]:
                    ...
                    menu.Append(wx.NewId(), _("Change POU Type To"), change_menu)
                new_id = wx.NewId()
    ...
    def OnDClick(self, event):
        ...
        if not self.VariableTree.ItemHasChildren(selected_Item):
            self.ClearHighlights(SEARCH_RESULT_HIGHLIGHT)
            parent = self.VariableTree.GetItemText(self.VariableTree.GetItemParent(event.GetItem()))

            data = self.VariableTree.GetItemData(selected_Item)
            if parent == (_("Global Variables")):
                infos = ('C::config', 'var_local', data["index_var"], 'name')
            else:
                infos = ("P::" + parent, 'var_local', data["index_var"], 'name')
            if isinstance(infos, tuple):

                self.ShowSearchResult(infos, (0, 0), (0, 3))
        else:
            self.VariableTree.Toggle(selected_Item)
    ...
    def clean_tree(self, tree, root_item, item_list):
        item, cookie = tree.GetFirstChild(root_item)
        if item is not None:
            while item is not None:
                text = tree.GetItemText(item)
                if text not in item_list:
                    tree.Delete(item)
                item, cookie = tree.GetNextChild(root_item, cookie)

    def insert_item_sort(self, tree, root_item, name, num=1):
        index = SORT_ITEM_TREE_LIST.index(name) - num
        if index >= 0:
            search_name = SORT_ITEM_TREE_LIST[index]
            item = self.get_item_by_label(tree, search_name, root_item)
            if item is None:
    ...
    def GenerateVariableTreeBranch(self, root, infos, item_alone=False):

        to_delete = []
        item_name = infos["name"]
        if infos["type"] in ITEMS_UNEDITABLE:
            #if len(infos["values"]) == 1:
            #    return self.GenerateVariableTreeBranch(root, infos["values"][0], True)
            item_name = _(item_name)
        self.VariableTree.SetItemText(root, item_name)
        self.VariableTree.SetItemData(root, infos)


        item, root_cookie = self.VariableTree.GetFirstChild(root)
        for values in infos["values"]:
            if values["type"] not in ITEMS_UNEDITABLE or len(values["values"]) > 0:
                if item is None or not item.IsOk():
                    self.VariableTree.AppendItem(root, "")
                    item, root_cookie = self.VariableTree.GetNextChild(root, root_cookie)
                self.GenerateVariableTreeBranch(item, values)
                item, root_cookie = self.VariableTree.GetNextChild(root, root_cookie)
        while item is not None and item.IsOk():
            to_delete.append(item)
            item, root_cookie = self.VariableTree.GetNextChild(root, root_cookie)
        for item in to_delete:
            self.VariableTree.Delete(item)

    def SelectVariableTreeItem(self, tagname):
        result = False
        if self.VariableTree is not None:
            root = self.VariableTree.GetRootItem()
            if root is not None and root.IsOk():
                words = tagname.split("::")
                result = self.RecursiveVariableTreeItemSelection(
                    root, list(zip(words[1:], self.TagNamePartsItemTypes.get(words[0], []))))
        return result

    def RecursiveVariableTreeItemSelection(self, root, items):
        found = False
        item, root_cookie = self.VariableTree.GetFirstChild(root)
        while item is not None and item.IsOk() and not found:
            item_infos = self.VariableTree.GetItemData(item)
            if (item_infos["name"].split(":")[-1].strip(), item_infos["type"]) == items[0]:
                if len(items) == 1:
                    self.SelectedItem = item
                    self.VariableTree.SelectItem(item)
                    self.ResetSelectedItem()
                    return True
                else:
                    found = self.RecursiveVariableTreeItemSelection(item, items[1:])
            else:
                found = self.RecursiveVariableTreeItemSelection(item, items)
            item, root_cookie = self.VariableTree.GetNextChild(root, root_cookie)
        return found

    def VariableTreeItemSelect(self, select_item):
        if select_item is not None and select_item.IsOk():
            name = self.VariableTree.GetItemText(select_item)
            item_infos = self.VariableTree.GetItemData(select_item)
            if item_infos["type"] in [ITEM_DATATYPE, ITEM_POU,
                                      ITEM_CONFIGURATION, ITEM_RESOURCE,
                                      ITEM_TRANSITION, ITEM_ACTION]:
                self.EditProjectElement(item_infos["type"], item_infos["tagname"], True)
    ...
    def SetDebugPage(self):
        select_page = self.RightNoteBook.GetSelection()
        if self.RightNoteBook.GetPageText(select_page) != _("Debugger"):
            for page_index in range(self.RightNoteBook.GetPageCount()):
                if self.RightNoteBook.GetPageText(page_index) == _("Debugger"):
                    self.RightNoteBook.SetSelection(page_index)
                    return
                ...
                for radio, modes, id, method, picture, help in EditorToolBarItems[menu]:
                    if modes & self.DrawingMode:
                        if radio or self.DrawingMode == FREEDRAWING_MODE:
                            EditorToolBar.AddRadioTool(toolId=id, bitmap1=GetBitmap(picture), bmpDisabled=wx.NullBitmap, label="", shortHelp=help)
                        else:
    ...
    def OnAddDataTypeMenu(self, event):
        ...
        if tagname is not None:
            self._Refresh(TITLE, FILEMENU, EDITMENU, PROJECTTREE, VARIABLETREE)
    ...
    def OnAddConfigurationMenu(self, event):
        ...
        if tagname is not None:
            self._Refresh(TITLE, FILEMENU, EDITMENU, PROJECTTREE, POUINSTANCEVARIABLESPANEL, VARIABLETREE)
    ...
    def GenerateChangePouTypeFunction(self, name, new_type):
        def OnChangePouTypeMenu(event):
            ...
            if self.ProjectTree.GetItemData(selected)["type"] == ITEM_POU:
                ...
                self._Refresh(TITLE, EDITORTOOLBAR, FILEMENU, EDITMENU, PROJECTTREE, LIBRARYTREE, VARIABLETREE)
    ...
    def OnPastePou(self, event):
        ...
        if self.ProjectTree.GetItemData(selected)["type"] != ITEM_PROJECT:
            ...
            try:
                ...
                pou_type = {'Functions': 'function', 'Function Blocks': 'functionBlock', 'Programs': 'program'}[
                    pou_type]
            except:
                pou_type = None
        else:
    ...
    def OnRemoveDataTypeMenu(self, event):
        ...
        if self.ProjectTree.GetItemData(selected)["type"] == ITEM_DATATYPE:
            ...
            if self.CheckDataTypeIsUsedBeforeDeletion(name):
                ...
                if idx is not None:
                    self.TabsOpened.DeletePage(idx)
                self._Refresh(TITLE, EDITORTOOLBAR, FILEMENU, EDITMENU, PROJECTTREE, VARIABLETREE)
    ...
    def ShowHighlight(self, infos, start, end, highlight_type):
        ...
        self.SelectVariableTreeItem(infos[0])
        if infos[1] == "name":
