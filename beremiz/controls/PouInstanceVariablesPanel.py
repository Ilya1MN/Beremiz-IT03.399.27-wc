...
class CustomTreeCtrlWithRightImage(CT.CustomTreeCtrl):
    ...
    def GetItemRightImagesBBox(self, item):
        ...
        if len(rightimages) > 0:
            ...
            bbox_x = w - bbox_width
            bbox_y = int(item.GetY() + ((total_h > r_image_h) and [(total_h-r_image_h)/2] or [0])[0])
...
class PouInstanceVariablesPanel(wx.Panel):
    ...
    def __init__(self, parent, window, controller, debug):
        ...
        self.VariablesList.SetBackgroundColour(wx.Colour("WHITE"))
        ...
        self.DebugInstanceImageDisable = self.TreeRightImageList.Add(GetBitmap("debug_instance_disable"))
        ...
        if self.PouInfos is not None:
            ...
            for var_infos in self.PouInfos.variables:
                ...
                if var_infos.debug and self.Debug:
                    if self.Controller.IsPLCStarted():
                        right_images.append(self.DebugInstanceImage)
                    else:
                        right_images.append(self.DebugInstanceImageDisable)
    ...
    def RefreshButtons(self):
        ...
        self.DebugButton.Enable(enabled and self.PouInfos.debug and self.Debug and self.Controller.IsPLCStarted())

        root = self.VariablesList.GetRootItem()
        if root is not None and root.IsOk():
            item, item_cookie = self.VariablesList.GetFirstChild(root)
            while item is not None and item.IsOk():
                item.Enable(self.Controller.IsPLCStarted())
