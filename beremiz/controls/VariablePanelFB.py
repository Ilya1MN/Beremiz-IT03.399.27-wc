...
class VariablePanelFB(VariablePanel):
    def __init__(self, parent, window, controler, element_type, debug=False):
        ...
        self.DEFAULT_TYPE = self.Controler.GetFunctionBlockTypes()[0]
        ...
        def _RefreshButtons():
            if self:
                ...
                bodytype = self.Controler.GetEditedElementBodyType(self.TagName)
                self.AddButton.Enable(not self.Debug and bodytype in ["ST", "IL"])
    ...
    def RefreshView(self):
        ...
        for values in self.Controler.GetEditedElementInterfaceVars(self.TagName, debug=self.Debug):
            if values.Class != "External" and values.Type in fb_types:
    ...
    def on_cell_right_click(self, event):
        ...
        if bodytype in ["ST", "IL"]:
            super().on_cell_right_click(event)

    def on_cell_right_clickGrid(self, event):
        bodytype = self.Controler.GetEditedElementBodyType(self.TagName)
        if bodytype in ["ST", "IL"]:
            super().on_cell_right_clickGrid(event)
        ...
        if bodytype in ["ST", "IL"]:
            ...
            if not isinstance(KeyNames, list):
                ...
                try:
                    ...
                    if classVar in [_("External"), _("Global")] or typeVar in self.Controler.GetBaseTypes():
                        self.NoDataInsertDialog()
                        return
                    select_row = None
                            ...
                            for col in range(self.VariablesGrid.GetNumberCols()):
                                if key == self.VariablesGrid.GetColLabelValue(col):
                                    if key == _("Class"):
                                        ...
                                        if TableDict[key][row] not in self.Controler.GetFunctionBlockTypes():
                                            TableDict[key][row] = self.DEFAULT_TYPE
                                    try:
