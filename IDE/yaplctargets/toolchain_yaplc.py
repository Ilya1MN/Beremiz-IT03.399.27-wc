...
class toolchain_yaplc(toolchain_gcc):
    ...
    def build(self):
        ...
        if toolchain_gcc.build(self):
            ...
            objcpy = [self.toolchain_prefix + "objcopy", "--change-address", self.load_addr, "-O", "ihex",
                      self.exe_path, self.exe_path + ".hex"]
            ProcessLogger(self.CTRInstance.logger, objcpy).spin()

            objcpy = [self.toolchain_prefix + "objcopy", "--change-address", self.load_addr, "-O", "binary",
