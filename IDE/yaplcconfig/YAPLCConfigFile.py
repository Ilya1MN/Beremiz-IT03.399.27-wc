...
class YAPLConfigFile(CodeFile):
    ...
    def GetLocationsForGroup(self, group, parser, parent=None, args=[]):
        ...
        for grp in group.children():
            if parent:
                if grp.parametrized():
                    result = [x for x in parent['children'] if x['name'].startswith(grp.name())]
                    if result:
                        lst = list(args)
                    ...
                    for line in combined:
                        ...
                        result = [x for x in parent['children'] if x['name'].startswith(lname)]
                        if result:
                            lst = list()
    ...
    def GetVariableLocationTree(self):
        ...
        if self.YAPLCParser is not None:
            ...
            for group in parser.groups():
                ...
                result = [x for x in locations if x['name'].startswith(group.name())]
                if result:
                    lst = list()
                    lst.append(result[0]['location'])
